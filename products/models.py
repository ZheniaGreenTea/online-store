...
class Product(models.Model):
    ...
    pub_year = models.PositiveIntegerField(default=current_year(), validators=[MinValueValidator(1900), max_value_current_year])
    language_choice = (('укранский', 'укранский'),
                       ('английский', 'английский'), ('русский', 'русский'))
    language = models.CharField(
        "язык", max_length=250, blank=True, null=True, choices=language_choice)
    translator = models.ManyToManyField(
        Translator, verbose_name="переводчик", blank=True)
    ganre = models.ManyToManyField(Ganre, verbose_name="жанр")
    slug = models.SlugField('url', max_length=200, unique=True)
    description = models.TextField('описание', blank=True)
    total_pages = models.PositiveIntegerField(
        'количество страниц', blank=True, null=True)
